...
class CalcsTransmon:
    ...
    def dispersiveH_params_PT_O1(Pmj, Ωm, Ej):
        ...
        χ_O1 = 0.25 * Ωm @ Pmj @ inv(Ej) @ Pmj.T @ Ωm * 1000.0  # GHz to MHz

        f_O1 = (
            f_0 - 0.5 * np.ndarray.flatten(np.array(χ_O1.sum(1))) / 1000.0
    ...
    def transmon_get_all_params(Ej_MHz, Ec_MHz):
        ...
        Omega_MHz = sqrt(1.0 / (Lj_H * Cs_F)) * 1e-6  # MHz

...
def cost_camille(x0):
    x=x0[0]
    y=x0[1]
    return -(x+y-2)**2-x**2-0*np.cos(5*(x+y)+0.1)
...
def SGD(loss_f_and_g,x0,eta):
    fx_list=[]
    x=x0
    for i in range(40):
        fx,jac_fx=loss_f_and_g(x)
        fx_list.append(fx)
        jac_fx+=np.random.randn(len(jac_fx))*3
        x=x-eta*jac_fx
    return x,fx_list
...
x,fx_list=adam(loss_f_and_g,x0,bounds,eta=0.07)

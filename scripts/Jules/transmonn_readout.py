...
if new_geometry :
    ...
    Lj = pm.set_variable('8.50074nH')
    T_pad_width = pm.set_variable('0.042um')
    T_pad_spacing = pm.set_variable('0.025um')
    trm_length = pm.set_variable('0.4553858mm')
    T_fillet = pm.set_variable('0.005mm')
    #constrain port for readout 
    tune_ro = pm.set_variable('1.7mm')
    pos_constr_ro = pm.set_variable('2.8mm') #distance from transmon to curve of readout cable
    
    
    pos_x_mem = pm.set_variable('-2.5mm')
    pos_y_mem = pm.set_variable('2mm')
...
project_info = ProjectInfo(r"D:\Users\hqc\Documents\Jules\\",
                               ...
                               design_name   = 'HFSSDesign12',  # Name of the desgin file (string). "None" will get the current active one.
...
Lj = 8.60128830261565 
trm_length = 0.4542985943020692 #mm
tune_ro = 1.256584085297964 #mm
...
def cost3_camille (Lj, trm_length, tune_ro, capa_ro_length):
    x0 = [trm_length, Lj , tune_ro, capa_ro_length]
    if x0 in trm_ro_inputs :
        freq_ro = trm_ro_outputs[trm_ro_inputs.index(x0)][0]
        freq_trm = trm_ro_outputs[trm_ro_inputs.index(x0)][1]
        chi_trm = trm_ro_outputs[trm_ro_inputs.index(x0)][2]
        chi_mem_trm = trm_ro_outputs[trm_ro_inputs.index(x0)][3]
    else :
        trm_ro_inputs.append(x0)
        x0 = np.array(x0)
        ...
        trm_ro_outputs.append([freq_ro, freq_trm, chi_trm, chi_ro_trm])
